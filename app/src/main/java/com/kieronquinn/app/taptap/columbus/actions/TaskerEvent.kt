...
import com.kieronquinn.app.taptap.activities.ActivityConfigTaskerEventTriple
import com.kieronquinn.app.taptap.activities.TaskerEventUpdate
import com.kieronquinn.app.taptap.activities.TaskerEventUpdateTriple
...
class TaskerEvent(context: Context, whenGates: List<WhenGateInternal>) : ActionBase(context, whenGates) {
    ...
    override fun onTrigger(detectionProperties: GestureSensor.DetectionProperties?) {
        ...
        try {
            if(isTripleTap) {
                ActivityConfigTaskerEventTriple::class.java.requestQuery(context, TaskerEventUpdateTriple())
            }else{
