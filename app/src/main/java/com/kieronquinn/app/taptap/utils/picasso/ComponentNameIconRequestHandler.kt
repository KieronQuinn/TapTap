...
class ComponentNameIconRequestHandler(context: Context) : RequestHandler() {
    ...
    override fun load(request: Request, networkPolicy: Int): Result? {
        return try {
            Result(
                getFullResIcon(ComponentName.unflattenFromString(request.uri.toString().split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()[1])!!),
                DISK
            )
    ...
    private fun getFullResIcon(componentName: ComponentName): Bitmap {
        ...
        return when {
            applicationInfo == null -> fullResDefaultActivityIcon
            activityInfo != null -> getFullResIcon(activityInfo, applicationInfo)
            serviceInfo != null -> getFullResIcon(serviceInfo, applicationInfo)
            else -> getFullResIcon(applicationInfo)
        }
    }

    private fun getFullResIcon(info: ActivityInfo, applicationInfo: ApplicationInfo): Bitmap {
        try {
    ...
    private fun getFullResIcon(info: ServiceInfo, applicationInfo: ApplicationInfo): Bitmap {
        try {
            val resources = pm.getResourcesForApplication(info.packageName)
            if (resources != null) {
                val iconId = info.icon
